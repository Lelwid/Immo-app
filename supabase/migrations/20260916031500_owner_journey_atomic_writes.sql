...
begin
  ...
  if exists (select 1 from public.properties where user_id = current_user_id) then
    raise exception 'portfolio_already_initialized';
  end if;
  ...
  return jsonb_build_object('property_id', new_property_id, 'unit_count', cardinality(unit_ids));
end;
$$;
...
declare
  ...
  received_amount numeric := (p_transaction->>'amount_received')::numeric;
...
begin
  ...
  if nullif(transaction_id, '') is null or received_amount <= 0 then
    raise exception 'invalid_payment';
  end if;

  if jsonb_typeof(p_charges) <> 'array' or jsonb_array_length(p_charges) < 1
     or jsonb_typeof(p_allocations) <> 'array' or jsonb_array_length(p_allocations) < 1 then
    raise exception 'invalid_payment_allocation';
  end if;

  select sum((item->>'amount_allocated')::numeric)
  into allocated_amount
  from jsonb_array_elements(p_allocations) item;

  if allocated_amount is distinct from received_amount
     or exists (
       select 1
       from jsonb_array_elements(p_allocations) item
       where item->>'transaction_id' is distinct from transaction_id
          or (item->>'amount_allocated')::numeric <= 0
     ) then
    raise exception 'invalid_payment_allocation';
  end if;
  ...
  from jsonb_array_elements(p_charges) item
  ...
  insert into public.payment_transactions (
    id, user_id, property_id, lease_id, tenant_id, received_at,
    amount_received, method, reference, notes
  ) values (
    transaction_id,
    current_user_id,
    (p_transaction->>'property_id')::uuid,
    nullif(p_transaction->>'lease_id', '')::uuid,
    nullif(p_transaction->>'tenant_id', '')::uuid,
    (p_transaction->>'received_at')::date,
    received_amount,
    p_transaction->>'method',
    nullif(trim(p_transaction->>'reference'), ''),
  ...
  select
    ...
    transaction_id,
    item->>'rent_charge_id',
    (item->>'amount_allocated')::numeric
  from jsonb_array_elements(p_allocations) item;
end;
$$;

revoke all on function public.record_rent_payment(jsonb, jsonb, jsonb) from public, anon;
grant execute on function public.record_rent_payment(jsonb, jsonb, jsonb) to authenticated;
